...
- dendronetwork: it is now possible to choose which correlation to use
  for creating network(s): using the correlation after the
  Hollstein(1980) transformation r_hol or the correlation without any
  transformation (r). The default is now r_hol.

#### Bug Fixes

#### Deprecated and defunct

- moved folder with paper to separate repository:
  <https://github.com/RonaldVisser/dendroNetwork_paper>
...
- Tested with latest version of Cytoscape: 3.10.2
  (<https://cytoscape.org/download.html>) and all works fine
...
- updated all links to <https://github.com/ropensci/dendroNetwork> or
...
- `donttest{}` was added to RING_Visser_2021.R because testing takes to
  long for CRAN.

#### Continuous integration

- removed pkgdown deployment, and now relying on ROpenSci:
...
- added vignette: dendrochronological_data which describes the data to
  be used.
- minor corrections
...
- added check to determine if input graph is an igraph object for all
  functions that need an igraph as input
...
- corrected error in `wuchwerte()`. It is not using the `anos1` dataset
  from dplR anymore
...
- corrected examples in `cyto_create_gn_style` and
...
- replaced igraph::graph.data.frame() with
  igraph::graph_from_data_frame(), since the former is deprecated in
  igraph 2.0.0
- replaced
  [`igraph::is.simple`](https://r.igraph.org/reference/is.simple.html)
  with
  [`igraph::is_simple`](https://r.igraph.org/reference/simplify.html)
- replaced
  [`igraph::decompose.graph`](https://r.igraph.org/reference/decompose.graph.html)
  with
  [`igraph::decompose`](https://r.igraph.org/reference/decompose.html)
- correction to calls to functions
  [`grDevices::colorRampPalette`](https://rdrr.io/r/grDevices/colorRamp.html)
  and [`stats::pnorm`](https://rdrr.io/r/stats/Normal.html)
...
- correction in clique_community_names(\_par) functions, due to \[k\]
  seen as link
- added to cytoscape functions

#### Continuous integration

- added GitHub Action for BiocMananager
  (<https://lcolladotor.github.io/biocthis/index.html>)
- added more tests
